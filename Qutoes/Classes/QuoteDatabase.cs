namespace Qutoes.Classes
{
    public class QuotesDatabase
    {
        ...
        public Author FindAuthor(string authorName, string authorSurname)
        {
            foreach (Author author in authors.Values)
            {
                bool nameMatches = author.AuthorName.Equals(authorName, StringComparison.OrdinalIgnoreCase);
                bool surnameMatches = author.AuthorSurname.Equals(authorSurname, StringComparison.OrdinalIgnoreCase);
        ...
        public IList<Quote> GetAuthorQuotes(string authorName, string authorSurname)
        {
            IList<Quote> authorQuotes = new List<Quote>();
            Author author = FindAuthor(authorName, authorSurname);

            if(author == null) { return authorQuotes; }
        ...
        public static void Run()
        {
            ...
            var foundQuotes = quotes.GetAuthorQuotes("robert", "martin");
            ...
            var notFound = quotes.GetAuthorQuotes("Nikola", "Tesla");

            foreach(var quote in notFound)
            {
                Console.WriteLine(quote.Text);
            }
        }
